...
client.fetchMappedEntries(with: query) { result in
    guard let mappedCatsArrayResponse = result.value else { return }
    (mappedCatsArrayResponse: MappedArrayResponse<Cat>) in
